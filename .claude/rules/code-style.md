...
- 기본적으로 네이밍은 **누구나 알 수 있는 쉬운 단어**를 선택한다
  - 쓸데없이 어려운 고급 어휘를 피한다
- 변수는 camelCase를 기본으로 한다 (userEmail, userCellPhone)
- 패키지명은 단어가 달라지더라도 무조건 소문자를 사용한다 (frontend, useremail)
...
- 매개변수 타입이 메서드 시그니처에서 이미 드러나면, 메서드명에 타입을 중복해서 사용하지 않음
  ...
  - 단, 서로 다른 타입을 구분해야 할 때는 사용 (findUserByEmail, findUserByUsername, findUserByPhoneNumber)
