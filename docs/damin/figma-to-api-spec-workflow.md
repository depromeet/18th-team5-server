...
| UserType | `FOOD_ENTHUSIAST` | **`SEASONAL_GOURMET`** |
| ERD.md EnjoyType | `EMOTIONAL_CONTENT` | **`CULTURE_CONTENT`** (코드 기준이 맞음) |
...
명세를 작성하면서 ERD와 코드 사이의 불일치(EMOTIONAL_CONTENT vs CULTURE_CONTENT 등)를 자연스럽게 발견했다. 명세 작성을 단순한 문서화가 아니라 **코드·설계 리뷰의 트리거**로 활용할 수 있다.
